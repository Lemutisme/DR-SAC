...
class PQL_BCQ(object):
    def __init__(self, **kwargs):
                #  (state_dim, action_dim, max_state, min_action, max_action,
        ...
        self.max_action = torch.tensor(self.max_action, dtype=torch.float, 
                                       device=self.device)
        self.min_action = torch.tensor(self.min_action, dtype=torch.float, 
                                       device=self.device)
        
        self.actor = Actor(self.state_dim, self.action_dim, self.max_action, self.phi).to(self.device)
        ...
        self.vae = VAE(self.state_dim, self.action_dim, self.action_dim * 2, self.max_action, self.device).to(self.device)
        self.vae_optimizer = torch.optim.Adam(self.vae.parameters())

        # define VAE state (visitation)
        # latent_dim = state_dim * 2
        self.vae2 = VAE_state(self.state_dim, self.state_dim * 2, self.max_state, self.device).to(self.device)
    ...
    def train(self, replay_buffer, iterations, batch_size=100):
        ...
        for it in range(iterations):
            ...
            with torch.no_grad():
                ...
                target_Q = reward + not_done * score * self.gamma * target_Q + not_done * self.gamma * (1 - score) * self.vmin
    ...
    def save(self, EnvName):
        model_dir = Path(f"./models/FQI_model/{EnvName}")
        model_dir.mkdir(parents=True, exist_ok=True)

        torch.save(self.actor.state_dict(), model_dir / f"actor.pth")
        torch.save(self.critic.state_dict(), model_dir / f"critic.pth")
        torch.save(self.vae.state_dict(), model_dir / f"vae.pth")
        torch.save(self.vae2.state_dict(), model_dir / f"vae2.pth")

    def load(self, EnvName, load_path):
        model_dir = Path(get_original_cwd())/f"{load_path}/models/FQI_model/{EnvName}"
        ...
        self.vae.load_state_dict(torch.load(model_dir / f"vae.pth", map_location=self.device, weights_only=True))
        self.vae2.load_state_dict(torch.load(model_dir / f"vae2.pth", map_location=self.device, weights_only=True))
