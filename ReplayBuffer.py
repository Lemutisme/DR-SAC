...
class ReplayBuffer(object):
    def __init__(self, state_dim, action_dim, max_size, device, EnvIndex=None):
        ...
        # Normalize states for better HalfCheetah VAE training.
        # Actions are NOT normalized because they already lie in [-1, 1].
        ...
        # Keep the option to select normalized dimensions; for HalfCheetah, normalize all dimensions.
        self.normalize_dim = []
        if EnvIndex == 3:
             self.normalize_dim = range(state_dim)        

    # Add transition tuple (s, a, r, s_next, dw) to the buffer.
    ...
    def sample(self, batch_size, norm_only=False):
        ...
        # For VAE training, return normalized data only.
        if norm_only:
    ...
    def load(self, path, reward_adapt, reward_normalize, EnvIdex):
        ...
        # Apply reward engineering or normalization when loading the dataset.
        if reward_adapt:
    ...
    # State de-normalization function.
    # Used to rescale state samples generated by the VAE.
